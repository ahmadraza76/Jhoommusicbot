...
async def play_music(_, message: Message):
    ...
    if not results:
        await msg.edit_media(
            InputMediaPhoto(media=ERROR_IMG, caption="❌ **No Results**\n\nTry a different query."), 
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔍 Try Again", callback_data="search_menu")]])
        )
        return
    ...
    buttons.append([
        InlineKeyboardButton("🔍 More Results", callback_data="more_results"), 
        InlineKeyboardButton("❌ Close", callback_data="close_search")
    ])
    
    await msg.edit_media(
        InputMediaPhoto(media=SEARCH_IMG, caption=f"🔍 **Search Results**\n\n`{query}`\n\nSelect a track:"), 
...
async def play_video(_, message: Message):
            ...
            if chat_id in current_streams:
                ...
                try:
                    ...
                    await message.reply_photo(
                        photo=ERROR_IMG, 
                        caption=f"❌ **Voice Chat Error**\n\n`{str(e)}`\n\nCheck VC permissions.", 
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Retry", callback_data="retry_vplay")]])
                    )
        except Exception as e:
            await message.reply_photo(
                photo=ERROR_IMG, 
    ...
    for i, result in enumerate(results[:5], start=1):
        title = result.get('title', 'Unknown Video')[:30]
        if len(result.get('title', '')) > 30:
            title += "..."
        duration = await format_duration(result.get('duration', 0))
        buttons.append([InlineKeyboardButton(f"{i}. {title} ({duration})", callback_data=f"vplay_{result['id']}")])
...
async def live_stream(_, message: Message):
    ...
    if not (url.startswith('http') and ('m3u8' in url)):
        return await message.reply_photo(
            photo=ERROR_IMG, 
            caption="❌ **Invalid URL**\n\nProvide a valid M3U8 URL.", 
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ℹ️ Learn More", url="https://en.wikipedia.org/wiki/M3U")]])
        )
        ...
        if chat_id in current_streams:
            ...
            try:
                ...
                await message.reply_photo(
                    photo=ERROR_IMG, 
                    caption=f"❌ **Voice Chat Error**\n\n`{str(e)}`\n\nCheck VC permissions.", 
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Retry", callback_data="retry_live")]])
                )
    except Exception as e:
        await message.reply_photo(
            photo=ERROR_IMG, 
