    ...
    try:
        ...
        if user_id not in auth_users:
            ...
            await message.reply_photo(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👥 View Authorized", callback_data="show_auth_users")]])
            )
        else:
            await message.reply_photo(
                photo=AUTH_IMG, 
                caption=f"ℹ️ **Already Authorized**\n\nUser ID: `{user_id}`", 
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👥 View Authorized", callback_data="show_auth_users")]])
            )
    except ValueError:
        await message.reply_photo(
...
async def unauth_user(_, message: Message):
    ...
    try:
        ...
        if user_id in auth_users:
            ...
            await message.reply_photo(
                photo=AUTH_IMG, 
                caption=f"✅ **User Unauthorized**\n\nUser ID: `{user_id}`", 
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👥 View Authorized", callback_data="show_auth_users")]])
            )
        else:
            await message.reply_photo(
                photo=AUTH_IMG, 
                caption=f"ℹ️ **Not Authorized**\n\nUser ID: `{user_id}` not in list.", 
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("👥 View Authorized", callback_data="show_auth_users")]])
            )
    except ValueError:
        await message.reply_photo(
